...
const FretboardGame: React.FC = () => {
  const {
    targetNoteIndices,
    targetNoteNames,
    colorIndices, // <--- New prop from hook
    noteCount,
    updateNoteCount,
    anchorFret,
    windowStart,
  ...
  // No more side effects or local state for colors!

  // Explicit string thicknesses from High E (Index 0) to Low E (Index 5)
    ...
    <div className="flex flex-col items-center p-8 min-h-screen bg-white text-slate-900 w-full font-sans select-none">
      ...
      <div className="mb-8 w-full max-w-[1000px] flex flex-col md:flex-row justify-between items-end border-b-2 border-slate-900 pb-4">
        ...
        <div className="flex flex-col gap-4">
          ...
          {/* Configurable Note Count Stepper */}
          <div className="flex items-center gap-3">
             ...
             <div className="flex items-center border-2 border-slate-200 rounded-lg overflow-hidden">
               <button
                 onClick={() => updateNoteCount(-1)}
                 className="px-3 py-1 hover:bg-slate-100 font-bold text-slate-600 active:bg-slate-200"
               >-</button>
               <div className="px-3 py-1 font-mono font-bold text-slate-900 min-w-8 text-center border-x-2 border-slate-100">
                 {noteCount}
               </div>
               <button
                 onClick={() => updateNoteCount(1)}
        ...
        <div className="flex flex-col items-center grow px-8">
           ...
           <div className="flex items-baseline">
             {targetNoteNames.map((name, idx) => {
                // Use the random index provided by the hook to pick a color
                ...
                return (
                  <React.Fragment key={idx}>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-6xl font-black ${color.text}`}>
                        {name}
                      </span>
                    </div>
                    {/* Minimal Separator */}
                    {idx < targetNoteNames.length - 1 && (
      ...
      <div className="relative w-full max-w-[1000px] overflow-x-auto pb-4">
        ...
        <div className="flex pl-10 mb-1 min-w-[800px]">
          {Array.from({ length: 15 }).map((_, i) => (
            <div
              key={i}
              className={`flex-1 text-center text-xs font-mono transition-colors duration-300 ${i === anchorFret ? 'text-red-600 font-bold' : 'text-slate-400'}`}
            >
              {i}
            </div>
          ))}
        ...
        <div className="relative border-y-12 border-[#5D4037] bg-slate-100 shadow-sm min-w-[800px]">
          ...
          {/* Fret Vertical Lines & Background Zoning */}
              ...
              return (
                <div
                  ...
                  className={`
                    ...
                    ${fret === 0 ? 'border-r-[6px] border-slate-800' : ''} /* The Nut */
                    ${isActive ? 'bg-white' : 'bg-slate-100 opacity-60'} /* Focus Window */
                  `}
                >
                   {/* Inlay Dots */}
              ...
              <div key={sIdx} className="relative h-12 flex items-center">
                ...
                <div className="flex w-full h-full pl-10">
                  {Array.from({ length: 15 }).map((_, fret) => {
                    ...
                    const isAnchorPos = sIdx === 0 && fret === anchorFret;

                    // --- LOGIC ---
                    const pitch = TUNING[sIdx] + fret;
                    const targetIndex = targetNoteIndices.indexOf(pitch % 12);
                    const isTarget = targetIndex !== -1;

                    // Resolve color using the hook's random indices
                    let colorTheme = null;
                    if (isTarget) {
                      const colorIndex = colorIndices[targetIndex % colorIndices.length];
                      colorTheme = SAFE_PALETTE[colorIndex];
                    }
                    ...
                    if (gameState === 'GUESSING') {
                      ...
                      if (isTarget && isClicked) {
                        ...
                        // MISSED (Ghost) - Filled, small, transparent
                      ...
                      >
                         ...
                         {isAnchorPos && (
                           <div className="absolute w-3 h-3 bg-red-600 rounded-sm z-0 opacity-80"
                                title="Melody Anchor Constraint" />
                         )}
      ...
      <div className="mt-10 flex flex-col items-center gap-4">
        <p className="text-slate-400 text-sm font-medium">
          {gameState === 'GUESSING'
            ? `Locate all target notes within the bright window.`
            : `Round Complete.`}
        </p>
        ...
        {gameState === 'GUESSING' ? (
          <button
            onClick={submitGuess}
            className="px-12 py-3 bg-slate-900 text-white text-lg font-bold tracking-wide hover:bg-slate-700 transition-colors active:transform active:scale-95 shadow-xl rounded-sm"
          >
            CHECK ANSWER
          </button>
        ) : (
          <button
            ...
            className="px-12 py-3 bg-white text-slate-900 border-2 border-slate-900 text-lg font-bold tracking-wide hover:bg-slate-50 transition-colors active:transform active:scale-95 shadow-xl rounded-sm flex items-center gap-2"
          >
