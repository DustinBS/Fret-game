...
export const GalleryMode: React.FC = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen overflow-hidden bg-white text-slate-900 font-sans select-none">
      ...
      {/* MAIN GALLERY */}
      <main className="flex-1 h-full overflow-y-auto flex flex-col py-1 p-4 lg:p-8 gap-4 min-w-0 bg-slate-100/50">
            ...
            <table className="w-full text-left border-collapse min-w-[800px]">
                ...
                <tbody className="divide-y divide-slate-100">
                    {CHORD_DICTIONARY.map(def => (
                        <tr key={def.quality} className="hover:bg-slate-50">
                            ...
                            {stringShapes.map(str => {
                                ...
                                let rootFret = (rootObj.pitchClass - (stringOpenPitch % 12) + 12) % 12;
