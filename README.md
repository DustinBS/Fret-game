# Fretboard Focus: Developer Overview

**Project Type:** Educational Web Game for Classical Guitar
...
The goal is to practice **Positional Recognition**: identifying notes relative to a fixed hand position rather than calculating absolute coordinates.

## 2\. Core Mechanics

  * **The Window:** Every round generates a random **Anchor Fret**. The playable area is restricted to `[Anchor - 3, Anchor + 3]`.
  * **The Anchor Constraint:** The "Index Finger" position (Fret `N` on the 1st String) is visually marked with a **Red Square**. This simulates the physical hand anchor.
...
### `src/hooks/useFretboardGame.ts` (The Brain)

Contains all game logic, state management, and strict separation of side effects.
...
### `src/components/FretboardGame.tsx` (The Face)

A "Schematic" style visualization component. It uses standard HTML/CSS borders to simulate a clean sheet-music aesthetic.
